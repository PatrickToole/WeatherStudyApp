...
def begin():
    ...
    Light_total_energy = (Light_total_intensity *
                          math.pi * (radius ** 2)) / 105
    ...
    workbook = xlsxwriter.Workbook(fileName.rstrip(
        '.csv') + 'processed.xlsx', {'nan_inf_to_errors': True})
    ...
    # formatting
    time_column_format = workbook.add_format(
        {'bg_color': '#D9D9D9', 'bold': True})
    raw_column_format = workbook.add_format({'bg_color': '#C5D9F1'})
    raw_column_title_format = workbook.add_format(
        {'bg_color': '#C5D9F1', 'bold': True})
    hour_column_format = workbook.add_format({'bg_color': '#D8E4BC'})
    hour_column_title_format = workbook.add_format(
        {'bg_color': '#D8E4BC', 'bold': True})
    day_column_format = workbook.add_format({'bg_color': '#E6B8B7'})
    day_column_title_format = workbook.add_format(
        {'bg_color': '#E6B8B7', 'bold': True})
    ...
    for item in range(len(time_delta)):
        worksheet_2.write(
            item + 1, 0, round(float(time_delta[item]), 2), time_column_format)
        worksheet_2.write(
            item + 1, 1, round(float(temperature[item]), 1), raw_column_format)
        worksheet_2.write(
            item + 1, 2, round(float(rolling_water_temp_hour[item]), 1), hour_column_format)
        worksheet_2.write(
            item + 1, 3, round(float(rolling_water_temp_day[item]), 1), day_column_format)
    ...
    for item in range(len(time_delta_air)):
        worksheet_3.write(
            item + 1, 0, time_delta_air[item], time_column_format)
        worksheet_3.write(
            item + 1, 1, round(float(air_temperature[item]), 1), raw_column_format)
        worksheet_3.write(
            item + 1, 2, round(float(rolling_air_temp_hour[item]), 1), hour_column_format)
        worksheet_3.write(
            item + 1, 3, round(float(rolling_air_temp_day[item]), 1), day_column_format)
